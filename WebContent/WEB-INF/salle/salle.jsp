		...
		<fieldset>
			...
			<table class="table table-striped">
				<tr>
					...
					<th>Liste de matieres exclues</th>
					<th></th>
					<th></th>
				...
				<c:forEach items="${salles}" var="salle">
					<tr>
						...
						<td>${salle.etablissement.nom}</td>
						<c:choose>
						    <c:when test="${salle.matieresExclues.size()==0}">
						        <td>Aucune(s)</td>
						    </c:when>    
						    <c:otherwise>
						    	<c:forEach items="${salle.matieresExclues}" var="matiereEx">
								    <td>${matiereEx}</td>
								</c:forEach>
						    </c:otherwise>
						</c:choose>
						
						<td><a href="edit?id=${salle.id}" class="btn btn-info btn-sm"><span class="glyphicon glyphicon-pencil"></span></a></td>
						<td><a href="delete?id=${salle.id}" class="btn btn-danger btn-sm"><span class="glyphicon glyphicon-trash"></span></a></td>
					</tr>
				</c:forEach>
